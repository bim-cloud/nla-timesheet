// ── NLA Export Module ─────────────────────────────────────────
// Generates professional Excel and PDF timesheets with NLA branding
// Requires: SheetJS (XLSX) + jsPDF + jsPDF-AutoTable loaded in index.html
...
const NLA_NAVY  = '#102347';
const NLA_GOLD  = '#C9A84C';
const NLA_LIGHT = '#E8EDF4';
const NLA_WHITE = '#FFFFFF';
...
// ── Excel Export ──────────────────────────────────────────────
async function exportToExcel(user, weekOffset = 0) {
  const dates   = getWeekDates(weekOffset);
  const dayLabels = dates.map(d => new Date(d + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' }));
  ...
  const colTotals = [0,1,2,3,4].map(ci => projects.reduce((s,r) => s + r.hours[ci], 0));
  ...
  // Header rows (will style later with !merges)
  ...
  wsData.push(['Status:', entries.some(e => e.status === 'submitted') ? 'Submitted' : 'Draft', '', '', '', '', '']);
  wsData.push(['']);

  // Column headers
  wsData.push(['Project', ...dayLabels, 'Total']);
  ...
  projects.forEach(p => {
    wsData.push([
      p.name,
      ...p.hours.map(h => h > 0 ? h : ''),
      { f: `SUM(B${wsData.length + 1}:F${wsData.length + 1})` },
    ]);
  });

  // Totals row
  const dataStart = 9;
  const dataEnd   = dataStart + projects.length - 1;
  wsData.push([
    'DAILY TOTAL',
    ...([0,1,2,3,4].map(ci => ({ f: `SUM(${['B','C','D','E','F'][ci]}${dataStart}:${['B','C','D','E','F'][ci]}${dataEnd})` }))),
  ...
  // Column widths
  ws['!cols'] = [
    { wch: 38 }, { wch: 14 }, { wch: 14 }, { wch: 14 }, { wch: 14 }, { wch: 14 }, { wch: 12 },
  ];
  ...
  const rawEntries = await window.SupaEntries.forWeekGrid(user.id, dates[0]);
  const ws2Data = [
    ['Nature Landscape Architects — Daily Entries'],
    [`Employee: ${user.name}  |  Week: ${weekLabel}`],
    [''],
    ['Date', 'Project', 'Task / Description', 'Type', 'Hours', 'Status'],
  ];

  const sorted = [...rawEntries].sort((a,b) => a.date.localeCompare(b.date) || a.created_at.localeCompare(b.created_at));
  sorted.forEach(e => {
    ws2Data.push([
      fmtDate(e.date),
      e.project_name || e.project_id,
      e.title,
      e.task_type === 'grid' ? 'Timesheet' : e.task_type,
      parseFloat(e.hours),
  ...
  ws2Data.push(['']);
  ws2Data.push(['', '', '', 'TOTAL', { f: `SUM(E5:E${ws2Data.length - 1})` }, '']);
  ...
  // ── Download ──────────────────────────────────────────────
  const filename = `NLA_Timesheet_${user.name.replace(/\s+/g,'_')}_${dates[0]}.xlsx`;
  XLSX.writeFile(wb, filename);
}

// ── PDF Export ────────────────────────────────────────────────
async function exportToPDF(user, weekOffset = 0) {
  ...
  // Fetch data
  const entries = await window.SupaEntries.forWeekGrid(user.id, dates[0]);
  const isSubmitted = entries.some(e => e.status === 'submitted');
  ...
  // ── Header band ───────────────────────────────────────────
  doc.setFillColor(16, 35, 71); // NLA Navy
  doc.rect(0, 0, PW, 28, 'F');

  // Logo
  try {
    doc.addImage(NLA_LOGO_B64, 'PNG', 8, 4, 20, 20);
  ...
  doc.setFontSize(14);
  doc.text('Nature Landscape Architects', 32, 13);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(180, 195, 220);
  doc.text('Weekly Timesheet Report', 32, 19);

  // Status badge (top right)
  const badgeColor = isSubmitted ? [29, 142, 74] : [180, 140, 30];
  doc.setFillColor(...badgeColor);
  doc.roundedRect(PW - 40, 9, 32, 9, 2, 2, 'F');
  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.text(isSubmitted ? 'SUBMITTED' : 'DRAFT', PW - 24, 14.5, { align: 'center' });

  // ── Info bar ──────────────────────────────────────────────
  doc.setFillColor(232, 237, 244);
  doc.rect(0, 28, PW, 16, 'F');

  doc.setTextColor(16, 35, 71);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  ...
  infoItems.forEach((item, i) => {
    const x = 8 + i * (PW - 16) / infoItems.length;
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(100, 120, 150);
    doc.setFontSize(6.5);
    doc.text(item[0].toUpperCase(), x, 33);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(16, 35, 71);
    doc.setFontSize(8.5);
    doc.text(item[1], x, 39);
  });

  // ── Weekly grid table ─────────────────────────────────────
  const tableHead = [['Project', ...dayLabels, 'Total']];
  const tableBody = projects.map(p => [
    p.name,
    ...p.hours.map(h => h > 0 ? h.toFixed(2) : '—'),
    p.hours.reduce((s,x) => s+x, 0) > 0 ? p.hours.reduce((s,x) => s+x, 0).toFixed(2) : '—',
  ]);

  tableBody.push([
    'DAILY TOTAL',
    ...colTotals.map(h => h > 0 ? h.toFixed(2) : '—'),
    grand > 0 ? grand.toFixed(2) : '—',
  ]);
  ...
  doc.autoTable({
    ...
    columnStyles: {
      ...
      4: { halign: 'center' },
      5: { halign: 'center' },
      6: { halign: 'center', fontStyle: 'bold', textColor: [16, 35, 71] },
    },
    alternateRowStyles: { fillColor: [245, 248, 252] },
    didParseCell: (data) => {
      // Highlight totals row
      if (data.row.index === tableBody.length - 1) {
        data.cell.styles.fillColor = [16, 35, 71];
        data.cell.styles.textColor = [255, 255, 255];
        data.cell.styles.fontStyle = 'bold';
      }
      // Highlight total column
      if (data.column.index === 6 && data.row.index < tableBody.length - 1) {
  ...
  // ── Daily entries table (page 2) ──────────────────────────
  doc.addPage();

  // Header band page 2
  doc.setFillColor(16, 35, 71);
  doc.rect(0, 0, PW, 18, 'F');
  try { doc.addImage(NLA_LOGO_B64, 'PNG', 8, 2, 13, 13); } catch(e) {}
  doc.setTextColor(255,255,255);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.text('Daily Time Entries', 26, 10);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.setTextColor(180, 195, 220);
  doc.text(`${user.name}  ·  ${weekLabel}`, 26, 15);

  const sorted = [...entries].sort((a,b) => a.date.localeCompare(b.date));
  const entriesBody = sorted.map(e => [
    fmtDate(e.date),
    e.project_name || e.project_id,
    e.title,
    e.task_type === 'grid' ? 'Timesheet' : (e.task_type || '—'),
  ...
  doc.autoTable({
    head: [['Date', 'Project', 'Task / Description', 'Type', 'Hours', 'Status']],
    body: entriesBody.length > 0 ? entriesBody : [['No entries found', '', '', '', '', '']],
    startY: 24,
    margin: { left: 8, right: 8 },
    styles: { font: 'helvetica', fontSize: 8, cellPadding: 3, textColor: [30, 40, 60] },
    headStyles: { fillColor: [16, 35, 71], textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 7.5 },
    columnStyles: {
      0: { cellWidth: 28 },
      1: { cellWidth: 55 },
      2: { cellWidth: 'auto' },
      3: { cellWidth: 24, halign: 'center' },
      4: { cellWidth: 18, halign: 'center', fontStyle: 'bold', textColor: [16, 35, 71] },
    ...
    didParseCell: (data) => {
      if (data.column.index === 5 && data.section === 'body') {
        ...
        if (val === 'Approved') { data.cell.styles.textColor = [29, 142, 74]; data.cell.styles.fontStyle = 'bold'; }
        if (val === 'Submitted') { data.cell.styles.textColor = [180, 140, 30]; data.cell.styles.fontStyle = 'bold'; }
        if (val === 'Draft') { data.cell.styles.textColor = [100, 120, 150]; }
      }
    },
  });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Page ${i} of ${pageCount}`, PW - 8, PH - 4, { align: 'right' });
    doc.setTextColor(201, 168, 76);
    doc.text('NLA Timesheet', PW / 2, PH - 4, { align: 'center' });
  }

  // ── Save ──────────────────────────────────────────────────
  const filename = `NLA_Timesheet_${user.name.replace(/\s+/g,'_')}_${dates[0]}.pdf`;
  doc.save(filename);
}

// ── Export buttons component ──────────────────────────────────
function ExportButtons({ user, weekOffset = 0, size = 'sm' }) {
  ...
  const handleExport = async (type) => {
    ...
    setExporting(type);
    try {
      if (type === 'excel') await exportToExcel(user, weekOffset);
      if (type === 'pdf')   await exportToPDF(user, weekOffset);
    } catch(e) {
...
// ── Manager: Export full team timesheet ───────────────────────
async function exportTeamExcel() {
  ...
  const [weekRows, profiles] = await Promise.all([
  ...
  // Sheet per employee + summary sheet
  const summaryData = [
    ['Nature Landscape Architects — Team Timesheet Summary'],
    [`Week: ${weekLabel}  |  Generated: ${fmtDate(today)}`],
    [''],
    ['Employee', 'Role', ...dayLabels, 'Total', 'Target', 'Utilization'],
  ];

  employees.forEach(emp => {
    const empEntries = weekRows.filter(e => e.user_id === emp.id);
    ...
    summaryData.push([
      ...
      total > 0 ? total : '',
      40,
      total > 0 ? `${((total/40)*100).toFixed(1)}%` : '0%',
    ]);
  });

  const ws = XLSX.utils.aoa_to_sheet(summaryData);
  ws['!cols'] = [{ wch: 28 }, { wch: 22 }, ...Array(5).fill({ wch: 14 }), { wch: 10 }, { wch: 10 }, { wch: 14 }];
  ws['!merges'] = [
    { s: { r: 0, c: 0 }, e: { r: 0, c: 9 } },
    { s: { r: 1, c: 0 }, e: { r: 1, c: 9 } },
  ];
...
async function exportTeamPDF() {
  ...
  doc.setFillColor(16, 35, 71);
  doc.rect(0, 0, PW, 28, 'F');
  try { doc.addImage(NLA_LOGO_B64, 'PNG', 8, 4, 20, 20); } catch(e) {}
  doc.setTextColor(255,255,255);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(14);
  doc.text('Nature Landscape Architects', 32, 13);
  ...
  doc.setFontSize(8);
  doc.setTextColor(180,195,220);
  doc.text('Team Timesheet Report', 32, 19);
  doc.setTextColor(201,168,76);
  doc.setFontSize(9);
  doc.setFont('helvetica', 'bold');
  doc.text(weekLabel, PW - 8, 13, { align: 'right' });
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7);
  doc.setTextColor(180,195,220);
  doc.text(`Generated: ${fmtDate(today)}`, PW - 8, 20, { align: 'right' });

  // Summary stats bar
  doc.setFillColor(232, 237, 244);
  doc.rect(0, 28, PW, 14, 'F');
  const totalHrs = weekRows.reduce((s,e) => s + parseFloat(e.hours), 0);
  ...
  statsItems.forEach((item, i) => {
    const x = 8 + i * (PW - 16) / statsItems.length;
    doc.setFont('helvetica', 'normal'); doc.setTextColor(100,120,150); doc.setFontSize(6.5);
    doc.text(item[0].toUpperCase(), x, 33);
    doc.setFont('helvetica', 'bold'); doc.setTextColor(16,35,71); doc.setFontSize(9);
    doc.text(item[1], x, 39);
  });

  // Team table
  const tableHead = [['Employee', 'Role', ...dayLabels, 'Total', 'Util %']];
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor: [16,35,71], textColor: [255,255,255], fontStyle: 'bold', fontSize: 7.5, halign: 'center' },
    columnStyles: {
      ...
      7: { halign: 'center', fontStyle: 'bold', textColor: [16,35,71] },
  ...
  doc.setFillColor(16,35,71);
  doc.rect(0, PH - 10, PW, 10, 'F');
  doc.setTextColor(160,180,210); doc.setFont('helvetica','normal'); doc.setFontSize(6.5);
  doc.text('Nature Landscape Architects · Confidential · Management Use Only', 8, PH - 4);
  doc.setTextColor(201,168,76);
  doc.text('NLA Timesheet', PW/2, PH - 4, { align: 'center' });
  doc.setTextColor(160,180,210);
  doc.text(`Page 1 of 1`, PW - 8, PH - 4, { align: 'right' });
