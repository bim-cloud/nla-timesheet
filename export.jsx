...
// ── Excel Export ──────────────────────────────────────────────
async function exportToExcel(user, weekOffset) {
  ...
  wsData.push(['Status:', entries.some(function(e){return e.status==='submitted';}) ? 'Submitted' : 'Draft', '', '', '', '', '']);
  wsData.push(['']);
  wsData.push(['Project'].concat(dayLabels).concat(['Total']));

  projects.forEach(function(p) {
    const row = [p.name].concat(p.hours.map(function(h){ return h > 0 ? h : ''; }));
    row.push(p.hours.reduce(function(a,b){return a+b;},0));
    wsData.push(row);
  });

  const ds = 9, de = ds + projects.length - 1;
  wsData.push(['DAILY TOTAL',
    {f:'SUM(B'+ds+':B'+de+')'},
    {f:'SUM(C'+ds+':C'+de+')'},
    {f:'SUM(D'+ds+':D'+de+')'},
    {f:'SUM(E'+ds+':E'+de+')'},
  ...
  ws['!merges'] = [
    ...
    {s:{r:3,c:1},e:{r:3,c:2}},{s:{r:4,c:1},e:{r:4,c:2}},
  ];
  ...
  sorted.forEach(function(e) {
    ws2Data.push([
      fmtDate(e.date), e.project_name || e.project_id, e.title,
...
// ── PDF Export ────────────────────────────────────────────────
async function exportToPDF(user, weekOffset) {
  weekOffset = weekOffset || 0;
  ...
  infoItems.forEach(function(item, i) {
    ...
    doc.setFont('helvetica','bold'); doc.setTextColor(16,35,71); doc.setFontSize(8.5);
  ...
  doc.autoTable({
    head: tableHead, body: tableBody, startY: 48,
  ...
  doc.autoTable({
    ...
    columnStyles: {
      0:{cellWidth:28}, 1:{cellWidth:55}, 3:{cellWidth:24,halign:'center'},
...
function ExportButtons({ user, weekOffset, size }) {
  ...
  return (
    <>
      <button className={'btn btn-'+size} onClick={() => handleExport('excel')} disabled={!!exporting}
        style={{display:'flex',alignItems:'center',gap:5}}>
        <Icon name="download" size={13}/>
        {exporting === 'excel' ? 'Exporting…' : 'Excel'}
      </button>
      <button className={'btn btn-'+size} onClick={() => handleExport('pdf')} disabled={!!exporting}
...
async function exportTeamExcel() {
  ...
  const summaryData = [
    ...
    [''],
    ['Employee', 'Role'].concat(dayLabels).concat(['Total','Target','Utilization']),
  ];
  ...
  ws['!cols'] = [{wch:28},{wch:22},{wch:14},{wch:14},{wch:14},{wch:14},{wch:14},{wch:10},{wch:10},{wch:14}];
...
async function exportTeamPDF() {
  ...
  doc.setFillColor(16,35,71); doc.rect(0,0,PW,28,'F');
  try { doc.addImage(NLA_LOGO_B64,'PNG',8,4,20,20); } catch(e){}
  ...
  doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(180,195,220);
  doc.text('Generated: ' + fmtDate(today), PW-8, 20, { align:'right' });

  doc.setFillColor(232,237,244); doc.rect(0,28,PW,14,'F');
  const statsItems = [
  ...
  const tableHead = [['Employee','Role'].concat(dayLabels).concat(['Total','Util %'])];
  ...
  doc.autoTable({
    head: tableHead, body: tableBody.length > 0 ? tableBody : [['No data','','','','','','','']],
    startY: 46, margin:{left:8,right:8},
  ...
  doc.setFillColor(16,35,71); doc.rect(0,PH-10,PW,10,'F');
  doc.setTextColor(160,180,210); doc.setFont('helvetica','normal'); doc.setFontSize(6.5);
